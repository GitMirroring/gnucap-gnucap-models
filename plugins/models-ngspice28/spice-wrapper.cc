...
bool DEV_SPICE::do_tr()
{
  ...
  if (_sim->analysis_is_tran_dynamic()) {
    trace2("td", _sim->is_first_iteration(), _sim->_time0);
    if ((_time[1] == _time[2])) { untested();
    ...
    if (_sim->analysis_is_tran_static()) {
      trace2("st", _sim->is_first_iteration(), _sim->_time0);
...
extern "C" {
  ...
  public:
    FT_CURCKT() {
      junk.jobs = NULL;
      // 28 ci_curTask = reinterpret_cast<TSKtask*>(&junk);
